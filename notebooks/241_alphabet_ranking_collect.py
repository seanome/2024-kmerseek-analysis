...
def read_pair(tag: str, qname: str) -> dict:
    ...
    if not p.exists():
        return {
            f"{key}_shared_kmers": None,
            f"{key}_regions": None,
            f"{key}_longest": None,
            f"{key}_in_window": None,
        }
    ...
    if qname in GOLD_WINDOW:
        ...
        in_window = any(
            r["query_start"] < qhi
            and r["query_end"] > qlo
            and r["target_start"] < thi
            and r["target_end"] > tlo
            for r in regs
        )
    return {
        f"{key}_shared_kmers": len(d.get("shared_kmers", [])),
...
def read_search(tag: str) -> pl.DataFrame | None:
    ...
    df = df.with_columns(
        ...
        pl.col("target_name")
        .map_elements(gene_symbol, return_dtype=pl.Utf8)
        .alias("gene"),
    )
    # A region with no lambda has no score scale, so its bit score comes back as 0 for
    # every region of the arm. Ranking on that ties the whole proteome at 0 and hands
    # rank 1 to whichever target sorts first: gbmr7 k=10 put BCL2 at rank 1 with 18_302
    # targets tied. Same trap as query_tfidf. Null it out so those arms produce no rank.
    if {"region_ka_bits", "region_ka_lambda"} <= set(df.columns):
        df = df.with_columns(
            pl.when(pl.col("region_ka_lambda") > 0)
            .then(pl.col("region_ka_bits"))
...
def rank_rows(df: pl.DataFrame, alphabet: str, k: int, bits: float) -> list[dict]:
    ...
    for q in QUERIES:
        ...
        for col, lower, label in METRICS:
            ...
            if partner is None:
                rec.update(
                    partner_found=None, rank=None, n_tied=None, partner_value=None
                )
            else:
                hit = per.filter(pl.col("gene") == partner)
                if hit.height == 0:
                    rec.update(
                        partner_found=False, rank=None, n_tied=None, partner_value=None
                    )
                else:
                    pv = float(hit["v"][0])
...
def main() -> None:
    ...
    for a, k, bits, c, x in plan.select(
        ...
        arm.update(
            match_probability_from_log(OUT / "logs" / f"{tag}.index.log", float(c))
        )
        arms.append(arm)
        ...
        regions.append(
            df.with_columns(
                ...
                pl.lit(k).alias("ksize_arm"),
                pl.lit(bits).alias("bits"),
            )
        )
    ...
    if regions:
        ...
        pl.concat(
            [r.select([c for c in keep if c in r.columns]) for r in regions],
            how="diagonal",
        ).write_parquet(OUT / "regions.parquet")
    print(
        f"arms: {len(arms)}  rank rows: {len(ranks)}  "
        f"searched: {sum(a['searched'] for a in arms)}",
        file=sys.stderr,
    )
