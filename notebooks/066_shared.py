...
def run_auc_sweep(
    exclude_gray_zone: bool, ref_queries: set, force_recompute: bool = False
) -> pl.DataFrame:
...
def plot_auc_heatmap(auc_df: pl.DataFrame, foldseek, tea_all, fig_prefix: str) -> None:
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_all_metrics_auc_heatmap.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def get_best_metric(auc_df: pl.DataFrame, foldseek, tea_all) -> dict:
    ...
    print(
        f'  Superfamily AUC: {best_fam_row["auc_sfam"]:.4f}  '
    ...
    print(
        f'  Superfamily AUC: {best_sfam_row["auc_sfam"]:.4f}  '
        f"(FoldSeek: {fs_sfam:.4f}, TEA: {tea_sfam:.4f})"
    )
...
def plot_best_metric_sensitivity(
    foldseek,
    tea_all,
    ref_queries: set,
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_best_metric_sensitivity_curve.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
    return best_rocx
...
def compute_k_comparison_rocx(
    foldseek, tea_all, ref_queries: set, exclude_gray_zone: bool
):
...
def plot_k_comparison(
    rocx_by_k: dict, foldseek, tea_all, ref_queries: set, fig_prefix: str
) -> None:
    fig, axes = plt.subplots(1, 4, figsize=(28, 6))
    for ax, (lc, ll) in zip(axes, [(lc, ll) for lc, _, ll in SCOP_LEVELS]):
        if lc != "CLASS":
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_k24_k27_sensitivity_comparison.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def plot_shared_queries(
    foldseek, tea_all, ref_queries: set, exclude_gray_zone: bool, fig_prefix: str
) -> None:
    ...
    for kc in K_COMPARE:
        ...
        n_ref = _rocx.filter(pl.col("NAME").is_in(ref_queries))["NAME"].n_unique()
        print(
            f"k={kc}: {n_all} covered | {n_ref} in ref | {len(_shared)} shared w/ FoldSeek"
        )
        del _ev
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_shared_queries_sensitivity.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def plot_metric_category_auc(
    auc_df: pl.DataFrame, foldseek, tea_all, fig_prefix: str
) -> None:
    _, _, fs_sfam = sensitivity_stats(foldseek, "SFAM")
    _, _, tea_sfam = sensitivity_stats(tea_all, "SFAM")
    ...
    for ax, (group_name, group_labels, color) in zip(
        ...
        ax.axvline(
            fs_sfam, color="black", ls="-", lw=2, label=f"FoldSeek ({fs_sfam:.3f})"
        )
        ax.axvline(
            tea_sfam, color="black", ls="--", lw=2, label=f"TEA ({tea_sfam:.3f})"
        )
...
def plot_auc_vs_ksize(auc_df: pl.DataFrame, foldseek, tea_all, fig_prefix: str) -> None:
    _, _, fs_sfam = sensitivity_stats(foldseek, "SFAM")
    _, _, tea_sfam = sensitivity_stats(tea_all, "SFAM")
    selected = [
        "BH (FDR)",
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_kmerseek_only_auc_vs_ksize.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def plot_precision_recall_curves(
    foldseek, tea_all, exclude_gray_zone: bool, fig_prefix: str
) -> None:
    _rocx_to_eval = {
        "FAM": "same_family",
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_precision_recall_curves.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def run_bonferroni_sweep(
    ...
    for k in KSIZES:
        try:
            ...
            print(
                f"k={k}: n={n_q}, fam={auc_fam:.3f}, sfam={auc_sfam:.3f}, fold={auc_fold:.3f}"
            )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_bonf_auc_vs_ksize_fam_sfam_fold.png",
        dpi=150,
        bbox_inches="tight",
    )
    plt.show()
...
def run_bonf_combo_analysis(
    bonf_df: pl.DataFrame,
    foldseek,
    tea_all,
    ref_queries: set,
    exclude_gray_zone: bool,
    ...
    ks_to_run = list(
        dict.fromkeys([best_fam_k, best_sfam_k])
    ...
    for k in ks_to_run:
        ...
        print(
            f'k={k}: n_queries={row["n_queries"]}, fam={auc_fam_cov:.4f}, '
...
def run_threshold_sweep(
    ref_queries: set, exclude_gray_zone: bool, force_recompute: bool = False
) -> pl.DataFrame:
    suffix = _cache_suffix(exclude_gray_zone)
...
def plot_threshold_auc_by_filter(
    thresh_auc_df: pl.DataFrame, foldseek, tea_all, fig_prefix: str
) -> None:
    _, _, fs_sfam = sensitivity_stats(foldseek, "SFAM")
    _, _, tea_sfam = sensitivity_stats(tea_all, "SFAM")
    ...
    style_handles = [
        Line2D(
            ...
            lw=5,
            ls="-",
            label=f"n_queries ≥ 10% ({NQ_10PCT:,})",
        ),
        Line2D(
            [0], [0], color="grey", lw=2, ls="-", label=f"n_queries ≥ 1% ({NQ_1PCT:,})"
        ),
        Line2D(
            [0], [0], color="grey", lw=1, ls=":", label=f"n_queries < 1% ({NQ_1PCT:,})"
        ),
        Line2D(
            [0], [0], color="black", lw=1.5, ls="-", label=f"FoldSeek ({fs_sfam:.3f})"
        ),
...
def plot_threshold_auc_by_metric(
    thresh_auc_df: pl.DataFrame, foldseek, tea_all, fig_prefix: str
) -> None:
    _, _, fs_sfam = sensitivity_stats(foldseek, "SFAM")
    _, _, tea_sfam = sensitivity_stats(tea_all, "SFAM")
    ...
    filter_handles = [
        Line2D(
            [0], [0], color=filter_color[fl], lw=2, marker="o", markersize=4, label=fl
        )
        for fl in all_filter_labels
    ]
    style_handles = [
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_sensitivity_curves_k25.png", dpi=150, bbox_inches="tight"
    )
    plt.show()


def run_threshold_comparison(
    thresh_auc_df: pl.DataFrame,
    foldseek,
    tea_all,
    ref_queries: set,
    exclude_gray_zone: bool,
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_bonferroni_correct_auc.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def plot_coverage_bins(
    ...
    for ax, (lo, hi, label) in zip(axes, cov_bins):
        sub = thresh_auc_df.filter(
            (pl.col("coverage") >= lo) & (pl.col("coverage") < hi)
        )
        if len(sub) == 0:
            ax.set_title(label + "\n(no data)")
            continue
        ...
        ax.axvline(
            tea_sfam, color="black", ls="--", lw=1.5, label=f"TEA ({tea_sfam:.3f})"
        )
    ...
    plt.suptitle(
        "Best Ranking Metric by Coverage Level", fontsize=13, fontweight="bold"
    )
    plt.tight_layout()
    plt.show()


def plot_best_combo_sensitivity(
    best_covered: dict,
    foldseek,
    tea_all,
    ref_queries: set,
    ...
    for ax, lc, ll in zip(axes, ["SFAM", "FAM"], ["Superfamily", "Family"]):
        ...
        ax.set_ylim(0, 1)
    plt.suptitle(
        "Best KmerSeek Threshold Combo vs FoldSeek & TEA",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
...
def run_combo_analysis(
    ...
    for row in _top.iter_rows(named=True):
        score_col, ascending = _score_map.get(
            row["score_label"], ("query_tfidf", False)
        )
        COMBOS.append(
            dict(
                k=row["ksize"],
...
def plot_class_enrichment(
    COMBOS: list, combo_results: dict, fs_cls: dict, fig_prefix: str
) -> None:
    all_cls = sorted(
        set(
            list(fs_cls)
    ...
    for ci, cfg in enumerate(COMBOS):
        ...
        ax.bar(
            x + offset_u,
            [u_cls.get(c, (0, 0))[1] * 100 for c in all_cls],
            w,
        ...
        ax.bar(
            x + offset_a,
            [a_cls.get(c, (0, 0))[1] * 100 for c in all_cls],
            w,
            color=colors_all[ci],
            alpha=0.7,
    ...
    ax.bar(
        x + offset_fs,
        [fs_cls.get(c, (0, 0))[1] * 100 for c in all_cls],
        w,
        color="steelblue",
        alpha=0.8,
        label="FoldSeek all",
    ...
    plt.tight_layout()
    plt.savefig(
        f"{fig_prefix}_unique_families_by_class.png", dpi=150, bbox_inches="tight"
    )
    plt.show()
...
def print_auc_comparison_table(
    ref_queries: set, foldseek, tea_all, exclude_gray_zone: bool
) -> None:
    combos = [
        (
            "n_queries ≥ 150",
    ...
    def km_auc_row(row):
        ...
        aucs = {
            level: (
                sensitivity_stats(rocx_cov, level)[2] if rocx_cov.height > 0 else 0.0
            )
    ...
    for name, df in [("FoldSeek", foldseek), ("TEA", tea_all)]:
        ...
        _, _, fold = sensitivity_stats(df, "FOLD")
        print(
            f'{name:<60} {fam:>9.4f} {sfam:>9.4f} {fold:>9.4f}  {df["NAME"].n_unique():,}'
        )
    print()
    for suffix, row in combos:
        ...
        aucs = km_auc_row(row)
        print(
            f'{label:<60} {aucs["FAM"]:>9.4f} {aucs["SFAM"]:>9.4f} {aucs["FOLD"]:>9.4f}'
