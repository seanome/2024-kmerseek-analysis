...
def accession_of(header: str, mode: str = "uniprot") -> str:
    """The join key for a FASTA header, in whichever form this pipeline keys on.

    `uniprot` is the qfo-pfam-region-benchmark behaviour, kept as the default so this
    copy stays drop-in compatible with that one: `sp|P04637|P53_HUMAN ...` -> `P04637`,
    falling back to the first whitespace-delimited token for headers that are not
    ...
    `verbatim` takes the first whitespace-delimited token and splits nothing. That is
    the ONLY mode the dark-set pipeline may use, because compute_dark_set.py keys its
    dark parquet on exactly that token. Under `uniprot` a QfO proteome's
    `tr|A0A024R1R8|A0A024R1R8_HUMAN` becomes `A0A024R1R8` while the dark parquet still
    says `tr|A0A024R1R8|A0A024R1R8_HUMAN`, so the two tables share no key at all and
    the join returns nothing -- silently, as "no dark protein has a disorder score".
    Botryllus happens to be immune (its `FUN000001_FUN000001` ids carry no '|', so both
    modes agree), which is what would have let this ship broken for every other species.
    """
...
def main() -> None:
    ...
    p.add_argument("--accession-mode", choices=("uniprot", "verbatim"), default="uniprot",
                   help="how to derive the join key from a FASTA header. 'uniprot' (default) "
                        "takes the middle field of a pipe-delimited id; 'verbatim' takes the "
                        "first whitespace token untouched, which is what compute_dark_set.py "
                        "keys on. See accession_of().")
    args = p.parse_args()
    ...
    for header, value in preds.items():
        ...
        if n == 0:
            continue
        acc = accession_of(header, args.accession_mode)
        # A repeated key means two records collapsed onto one accession, which would make
        # by_accession serve the wrong residue scores and put two same-keyed rows in the
        # parquet for a downstream join to multiply out. Caught here, naming the header,
        # rather than surfacing later as a row count that is quietly too large.
        if acc in by_accession:
            raise SystemExit(
                f"two records in {args.fasta.name} share the accession {acc!r} under "
                f"--accession-mode {args.accession_mode} (second one: {header!r}). "
                "Keys must be unique; re-run with --accession-mode verbatim if the "
                "collision comes from splitting on '|'."
            )
        by_accession[acc] = scores
        rows.append({
            "accession": acc,
