...
def write_empty_outputs(ksize: int, reason: str = 'empty file') -> None:
    ...
    for suffix in ['tsv', 'roc_data.tsv', 'mht.csv']:
        p = f'ortholog_evaluation.hp.k{ksize}.{suffix}'
        open(p, 'w').close()
        zst_compress(p)
    open(f'ortholog_evaluation.hp.k{ksize}.summary.txt', 'w').write(
        f'K-size: {ksize}\nEncoding: hp\n\nNo search results ({reason}).\n'
    )
    fig, ax = plt.subplots()
    ax.set_title(f'No results for k={ksize}')
    plt.savefig(f'metrics_no_results.hp.k{ksize}.png', dpi=72)
    plt.close()
...
def compute_mht(
    ...
    """
    mht_path = f'ortholog_evaluation.hp.k{ksize}.mht.csv'
...
def compute_roc_data(
    ...
    roc_path = f'ortholog_evaluation.hp.k{ksize}.roc_data.tsv'
...
def main() -> None:
    ...
    # ── Full evaluation TSV ────────────────────────────────────────────────────
    eval_path = f'ortholog_evaluation.hp.k{ksize}.tsv'
    if prefiltered:
