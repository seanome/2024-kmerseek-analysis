...
def transfer_keep_target(
    regions: pl.LazyFrame, domain_map: pl.LazyFrame, min_overlap: float, ev
) -> pl.LazyFrame:
    ...
    return (
        joined.with_columns(
            ev.overlap_expr("tstart", "tend", "t_feat_start", "t_feat_end").alias(
                "t_overlap"
            ),
...
def reduce_one(
    job: dict, truth: pl.DataFrame, domain_map: pl.LazyFrame, ev, min_overlap: float
) -> tuple[pl.DataFrame, pl.DataFrame]:
    regions = ev.load_regions(
        job["path"], False, rank_by="region_enrichment", max_bonferroni_p=0.05
    )
    if regions is None:
        return pl.DataFrame(), pl.DataFrame()
    if job["tool"] in FRAGMENT_TOOLS:
        regions = ev.dedup_fragment_regions(regions.collect(), 0.9).lazy()
    calls = transfer_keep_target(regions, domain_map, min_overlap, ev).collect(
        engine="streaming"
    )
    ...
    return (
        out.with_columns(**{k: pl.lit(v) for k, v in ident.items()}),
...
def main():
    ...
    ap.add_argument(
        "--results", type=Path, required=True, help="midi-plus results/ directory"
    )
    ap.add_argument(
        "--qfo-bin",
        type=Path,
    ...
    if args.list:
        ...
        print(
            f"{len(jobs)} input files: {n_km} kmerseek, {len(jobs) - n_km} comparison tools"
        )
        return
    ...
    truth = pl.read_parquet(
        args.results / "truth_swissprot" / "human_swissprot_truth.parquet"
    )
    ...
    for j in mine:
        ...
        if j["species"] not in maps:
            maps[j["species"]] = pl.read_parquet(
                args.results / "truth_swissprot" / f"{j['species']}_domain_map.parquet"
            ).lazy()
        ...
        for df, out in ((inst, out_i), (by_type, out_t)):
            ...
            tmp.rename(out)
        print(
            f"{stem}: {inst.height} instances overlapped, "
            f"{int(inst['landed'].sum()) if inst.height else 0} landed",
            flush=True,
        )
